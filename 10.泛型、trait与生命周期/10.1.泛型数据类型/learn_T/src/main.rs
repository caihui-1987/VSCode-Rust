//1.泛型是具体类型或者其他属性的抽象代表，用于减少代码重复
//2.在函数定义中使用泛型
//for i32
fn langest_i32(list: &[i32]) -> i32 {
...
//for char
fn langest_char(list: &[char]) -> char {
    let mut largest = list[0];
    for &item in list.iter() {
        if item > largest {
            largest = item;
...
//6.总结：使用泛型并不会造成程序性能上的损失。rust通过在编译时进行泛型代码的
//态化来保证效率。单态化时通过填充编译时使用的具体类型，将通用代码转换为特定代码的过程
